...
def best_partition(graph,
    ...
    >>> plt.show()
    """
    dendo, iteration = generate_dendrogram(graph,
                                partition,
                                weight,
                                resolution,
                                randomize,
                                random_state)
    return partition_at_level(dendo, len(dendo) - 1), iteration
...
def generate_dendrogram(graph,
    ...
    status_list = list()
    __one_level(current_graph, status, weight, resolution, random_state, iteration)
    ...
    while True:
        ...
        if new_mod - mod < (__MIN/iteration/__RATIO):
            break
        partition = __renumber(status.node2com)
        status_list.append(partition)
        mod = new_mod
...
def __one_level(graph, status, weight_key, resolution, random_state, iteration):
